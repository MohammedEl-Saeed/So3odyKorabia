            ...
            <div class="iq-card">
                ...
                <div class="iq-card-body">
                    <div class="new-user-info">
                        <form id="main-form-to-add-doctor" class="form" method="post" action="{{route('options.update', $item->id)}}" enctype="multipart/form-data">
                        @csrf
                            {{method_field('PUT')}}
                            <div class="row">
                                <div class="form-group col-md-6">
                                    <label for="name">الاسم:</label>
                                    <input type="text" class="form-control" id="name" name="name" value="{{old('name',$item->name)}}"  min="3"  max="100">
                                </div>
                                    <div class="col-md-6">
                                        <label>النوع:</label>
                                        <select  class="form-control options" name="type" value="{{old('type',$item->type)}}">
                                            @if(isset($types))
